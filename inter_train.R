...
# answer
data <-data |>
  mutate(new_attendance = case_when(type == '1'  ~ attendances * 0.5,
...
data_tot_perc <- data |>
  mutate (total_attend = sum(attendances),
          perc_attend = round((attendances / total_attend) * 100, 
                              1),
          .by = c(org_code, 
                  period)) 
...
data_roll <- data |>
  ...
  arrange(org_code,
          type,
          period) |>
  mutate(rolling_mean = rollapply(attendances,
                                  3,
                                  mean,
                                  align= 'right',
                                  fill = NA),
         rolling_median = rollapply(attendances,
                                     3,
                                    median,
                                    align= 'center',
                                    fill = 9999),
...
data_year <- data |>
  filter(org_code %in% c('RF4',
                         'RQ3'),
         type == '1') |>
  mutate(fin_year = if_else(month(period) >= 4, 
                               year(period) + 1, 
                               year(period))) |>
...
# answer
data_filt <- data |>
  filter(parse_number(as.character(org_code), 
                                  na = c("NA", 
                                       "Nothing")) > 50)
...
# answer
short_hospital_name <- str_replace(short_hospital_name,
                                   'P',
                                   'p')
...
plot_spc |> 
  ptd_create_ggplot(fixed_y_axis_multiple = TRUE,
                    x_axis_date_format = "%b %y",
                    x_axis_breaks = "2 months",
                    point_size = 2,
                    x_axis_label = 'Date',
                    main_title = paste0('SPC Chart of attendances for sites ', 
                                        toString(unique(data_spc$org_code))
                    ))
...
is_odd_or_even <- function (input, fact) {
  mult <- input * fact
  mod <- mult %% 2
  return (paste0(input, 
                 ' times ',
